...
class Classifier(nn.Module):
	...
	def forward(self, quan_eeg_top, quan_nirs_top, quan_eeg_bottom, quan_nirs_bottom):
		...
		features = torch.cat([eeg_fusion, nirs_fusion], dim=-1) # [16, 128]
...
class EFBook(nn.Module):
	...
	def __init__(self, depth, query_size, key_size, value_size, dict_len, emb_size, decay, num_heads, expansion, conv_dropout,
		...
		self.quantizer_top = Quantization(dict_len=256, emb_size=64)
		self.quantizer_middle = Quantization(dict_len=256, emb_size=128)
		self.quantizer_bottom = Quantization(dict_len=256, emb_size=256)
		self.fusion = ConditionalFusion()
		self.classifier = Classifier(emb_size, num_classes)

	def forward(self, eeg, nirs):
		temporal_eeg, temporal_nirs = self.temporal_conv(eeg, nirs) # [B, E, 1, T'] [B, E, 1, T'']
		temporal_eeg = temporal_eeg.squeeze(-2).permute(0,2,1)
		temporal_nirs = temporal_nirs.squeeze(-2).permute(0,2,1) # [B, T', E] [B, T'', E]

		eeg_token, nirs_token = self.transformer(temporal_eeg, temporal_nirs) # [B, E]

...
class EF_Book(nn.Module):
	...
	def forward(self, eeg, nirs, last_batch):
		...
		quan_eeg, eeg_quan_loss = eeg_quan_outputs['quan_token'], eeg_quan_outputs['quan_loss']
		...
		quan_nirs, nirs_quan_loss = nirs_quan_outputs['quan_token'], nirs_quan_outputs['quan_loss']
		...
		quan_fusion, fusion_quan_loss = fusion_quan_outputs['quan_token'], fusion_quan_outputs['quan_loss']
		...
		return {
			'outputs': outputs,
			'loss': 1000 * disentangling_loss + 0.15 * codebook_loss,
		}